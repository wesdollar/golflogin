<?php
if (!empty($tpl['ranking']['par_five_avg']) && $tpl['ranking']['par_five_avg'] && is_array($tpl['ranking']['par_five_avg'])) {
	?>
	<div class="box2B<?php echo $is_rankings ? ' marR8' : null; ?>">
		<div class="box2C">
			<table width="234" border="0" cellpadding="0" cellspacing="0">
				<tr>
					<th><img src="<?= WEB_URL; ?>img/h3_box2_13.2.gif" alt="" height="13" /></th>
				</tr>
			</table>
			<table width="222" border="0" cellpadding="0" cellspacing="0" class="marRL5">
				...
				<?php
				$i = 1;
				$total_rows = count($tpl['ranking']['par_five_avg']);
				
				foreach ($tpl['ranking']['par_five_avg'] as $key => $val) {
					?>
					<tr class="<?php echo $i%2 == 0 ? 'bgdBox203' : 'bgdBox202'; ?>">
						...
						<td valign="top" class="<?php echo $i == $total_rows ? 'borNone ' : null; ?>last"><?php echo $val['par_five_avg']; ?></td>
					</tr>
					<?php
					$i ++;
				}
