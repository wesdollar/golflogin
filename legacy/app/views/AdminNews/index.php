...
if (!empty($tpl['error'])) {
	...
	<div class="box">
		<h3><?php echo $GTM_LANG['AdminNews']['heading']; ?></h3>
		...
		if (is_array($tpl['arr']))
		{
			...
			if ($count > 0)
			{
				...
				<table class="middle_table">
					<thead>
						<tr>
							<th><?php echo $GTM_LANG['AdminNews']['title']; ?></th>
				...
				for ($i = 0; $i < $count; $i++)
				{
					?>
					<tr>
						<td><span class="editable"><?php echo readFromDb($tpl['arr'][$i]['news_title']); ?></span></td>
						<td><a class="icon icon_edit" href="<?php echo $_SERVER['PHP_SELF']; ?>?controller=AdminNews&amp;action=update&amp;id=<?php echo $tpl['arr'][$i]['id']; ?>"><?php echo $GTM_LANG['_edit']; ?></a></td>
						<td><a class="icon icon_delete" href="#" onclick="var q = confirm('<?php echo htmlspecialchars($GTM_LANG['_sure']); ?>'); if (q) { window.location = '<?php echo $_SERVER['PHP_SELF']; ?>?controller=AdminNews&amp;action=delete&amp;id=<?php echo $tpl['arr'][$i]['id']; ?>';}"><?php echo $GTM_LANG['_delete']; ?></a></td>
					</tr>
					<?php
				}
				?>
				...
				echo $GTM_LANG['Admin']['messages'][23];
			}
		}
	}
